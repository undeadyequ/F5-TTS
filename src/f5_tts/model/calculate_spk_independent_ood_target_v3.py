...
def _interp_unvoiced(f0: np.ndarray, unvoiced_pitch_max: float = 50.0) -> np.ndarray:
   """Linear interpolation over unvoiced (f0 <= unvoiced_pitch_max) frames.

   Numpy equivalent of _interp_unvoiced in cefm_v2.py.
   Args:
       f0: [T] F0 array in Hz, 0 for unvoiced frames
   Returns:
       [T] interpolated F0 array
   """
   f0_interp = f0.copy()
   voiced = f0 > unvoiced_pitch_max
   if voiced.any() and (~voiced).any():
       v_idx = np.where(voiced)[0]
       u_idx = np.where(~voiced)[0]
       pos = np.searchsorted(v_idx, u_idx)
       lo = np.clip(pos - 1, 0, len(v_idx) - 1)
       hi = np.clip(pos,     0, len(v_idx) - 1)
       span = np.maximum(v_idx[hi] - v_idx[lo], 1).astype(float)
       w = np.clip((u_idx - v_idx[lo]) / span, 0.0, 1.0)
       f0_interp[u_idx] = f0[v_idx[lo]] + w * (f0[v_idx[hi]] - f0[v_idx[lo]])
   return f0_interp
...
def process_single_line(args):
   ...
   try:
       ...
       if f0_hz is not None:
           f0 = np.where(f0_hz > 0, f0_hz, 0.0)   # JDC: 0 Hz for unvoiced
       else:
           ...
           f0 = np.where(voiced_flag, f0_raw, 0.0)  # pyin NaN → 0 for unvoiced

       if (f0 > 50.0).sum() < 2:
           return wav_id, {}

       # Step 1: Linear interpolation over unvoiced frames (f0 <= 50 Hz)
       f0_interp = _interp_unvoiced(f0, unvoiced_pitch_max=50.0)

       # Step 2: Semitone conversion
       f0_semi = 12.0 * np.log2(np.maximum(f0_interp, 1e-6) / 100.0)

       # Step 3a (for mu): Median filter (window=3) — mirrors _median_filter in cefm_v2.py
       f0_semi_median = scipy.ndimage.median_filter(f0_semi, size=3, mode='nearest')

       # Step 3b (for kappa): Savitzky-Golay smoothing on semitone values (window=7, polyorder=2)
       f0_semi_smooth = scipy.signal.savgol_filter(f0_semi, window_length=7, polyorder=2)

       # 4. Phoneme-level Aggregation
       ...
       for i, phone in enumerate(phones):
           ...
           if end - start < 2:
               continue
           # Step 4a: phoneme-level pitch (mu) — mean of median-filtered semitone values
           mu = np.mean(f0_semi_median[start:end])
           if np.isnan(mu):
               continue
           # Step 4b/5b: phoneme-level curvature (kappa) — |d²| of SG-smoothed semitone values
           d2 = np.gradient(np.gradient(f0_semi_smooth[start:end]))
...
def main(input_file, num_workers=12, fa_json=None, compute_fa=False, max_lines=10000,
   ...
   if start_step <= 1 <= end_step:
       if Path(mu_path).exists() and Path(kappa_path).exists():
           ...
           if use_jdc:
               device = "cuda" if torch.cuda.is_available() else "cpu"
               wav_paths = [line.strip().split('|')[0] for line in lines if line.strip().split('|')]
           ...
           args_list = [(line, get_fa_entry(line), jdc_f0_map.get(line.strip().split('|')[0])) for line in lines]  # INPUT: lines, fa, pitch

           ## 1.2 aggregate phoneme-level f0 (mu) and f0_curvature (kappa) for each *.wav
           print(f"Processing {len(lines)} files...")
           if num_workers > 1:
               with ProcessPoolExecutor(max_workers=num_workers) as executor:
                   results = list(tqdm(executor.map(process_single_line, args_list), total=len(args_list)))
           else:
   ...
   if start_step <= 2 <= end_step:
       ...
       ## 2.2 compute 0.55, 0.75, 0.95 quantiles of mu and kappa
   ...
   # ── Step 3: Cluster speakers by mu/kappa/mukappa ───────────────────────────────────────────────
   cluster_results = {}
   if start_step <= 3 <= end_step:
       cluster_results = cluster_speakers(spk_quantiles, suffix, speakers_txt=speakers_txt, cluster_by=cluster_by)
   ...
   # ── Step 4: Q55/Q75/Q95 of mu and kappa per cluster group ─────────────────────────────────
